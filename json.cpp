...
namespace json {
    ...
    bool Element::isString() const { return std::holds_alternative<std::string>(value); }

    bool Element::isObjectPtr() const { return std::holds_alternative<std::shared_ptr<Object>>(value); }

    bool Element::isVector() const { return std::holds_alternative<std::vector<Element>>(value); }
    ...
    std::shared_ptr<Object> Element::getObjectPtr() const { return std::get<std::shared_ptr<Object>>(value); }

    std::vector<Element> Element::getVector() const { return std::get<std::vector<Element>>(value); }
    ...
    std::variant<int, float> readNumber(const std::string &input, int &pos) { return parseNumber(input, pos); }
            ...
            if (Element element = readElement(root, input, pos); !element.isInt()) {
                if (element.isFloat()) {
                    if (float indexMaybe = element.getFloat();
                        to_string(indexMaybe) == std::to_string(static_cast<int>(indexMaybe))) {
    ...
    size_t findOperation(const std::string &input) {
        for (char operations[4] = {'+', '-', '*', '/'}; const char operation: operations) {
    ...
    float performOperation(const Object &root, const std::string &input, const size_t &op) {
        int pos = 0;
        std::string t = input.substr(0, op);
        const float a = makeFloat(readElement(root, t, pos));
        t = input.substr(op + 1);
        pos = 0;
        const float b = makeFloat(readElement(root, t, pos));
        switch (input[op]) {
            case '+':
                return a + b;
            case '-':
